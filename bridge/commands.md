...
Installation requires `--install`. A backup restores all three files if the
Live check fails. Keep staged devices and backups private and do not commit or
upload them. See `INSTALL.md` for paths and options.
...
There is no application-level authentication. The receiver is loopback-only,
so any process running as a user on this workstation can send commands while
the device is loaded. Inspect first, keep writes narrow, and read back changes.
