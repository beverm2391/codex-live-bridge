...
- Preserved Ableton device metadata during updates.
- Added a localhost status check after installation, reported
...
- Routed unmatched Max selectors through an explicit wrapper dispatcher.
...
- Removed the static capability token and its second Max JavaScript inlet. The
  bridge now relies on its explicit loopback-only local trust boundary.

### Changed

- Clarified the standalone bridge setup, its three-file device package, and
  the separate Live Extension workflow.
- Clarified the localhost trust boundary and inspect/write/readback workflow.
